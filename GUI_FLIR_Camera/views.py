...
class StartWindow(QMainWindow):
    def __init__(self, cam):
        ...
        self.edit_dn.setText(str(self.Phaseimg.dn))
        self.edit_wavelength.setText(str(self.Phaseimg.lamda))
        ...
        self.setting_layout.addWidget(self.label_mag,8,0,1,1)
        self.setting_layout.addWidget(self.edit_mag,8,1,1,2)
        self.setting_layout.addWidget(self.label_pixel_size,9,0,1,1)
        self.setting_layout.addWidget(self.edit_pixel_size,9,1,1,2)        
        ##
    ...
    def update_phase_or_height(self, converted_img):
        ...
        if (self.colormap_auto==0) and (self.colormap_min!=None) and (self.colormap_max!=None):
            self.converted_imageitem.setImage(self.Phaseimg.phase_image, autoLevels=False, levels=(self.colormap_min,self.colormap_max))
        else:
            self.converted_imageitem.setImage(self.Phaseimg.phase_image)
    ...
    def convertion(self):
        ...
        phase_worker=Image_retrieval(self.Phaseimg, self.mode)
        phase_worker.signals.emit_converted_img.connect(self.update_phase_or_height)
        if self.keep_convert==1:
            phase_worker.signals.finished.connect(self.convertion)
        self.threadpool.start(phase_worker)
    ...
    def change_dn(self):
        self.Phaseimg.dn=float(self.edit_dn.text())

    def change_wavelength(self):
        self.Phaseimg.lamda=float(self.edit_wavelength.text())
    ...
    def change_mag(self):
        if self.edit_mag.text()=='':
            ...
            self.mag=float(self.edit_mag.text())
    ...
    def change_pixel_size(self):
        if self.edit_pixel_size.text()=='':
            ...
            self.pixel_size=float(self.edit_pixel_size.text())
        self.Phaseimg.zoom_index=self.pixel_size/self.mag
